...
def create_dictionaries(path_to_dictionaries:list,verbose=True):
    ...
    Args:
        path_to_dictionaries(list):list of path of dictionaries
        ignore_log(bool, optional): ignore if multiple dictionaries are created.Default to False
...
def save_json(path:Path,data:dict):
    ...
    logger.info(f"json saved at path :{path}")
...
def save_bin(data:Any,path:Path):
    ...
    """
    joblib.jump(value=data,filename=path)
